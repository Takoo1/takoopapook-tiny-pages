  ...
  return <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50 pt-safe-top h-[56px] shadow-sm">
      <div className="flex items-center justify-between px-4 py-1.5 h-full">
        ...
        <div className="flex items-center gap-2 animate-fade-in" style={{ animationDelay: '0.1s' }}>
          ...
          {user ? (/* Logged in - Show FC Balance and Referral Button */
        <>
              <Button variant="ghost" onClick={handleWalletClick} className="flex items-center gap-1.5 px-3 py-1.5 bg-primary/10 rounded-full border border-primary/20 shadow-sm hover:bg-primary/15 transition-all duration-200 active:scale-95 animate-glow-pulse">
                <img src={fcCoin} alt="FC" className="w-5 h-5" />
                <span className="text-sm font-semibold text-primary">
                  {fcBalance.toLocaleString()}
                </span>
              </Button>
              
              <Button variant="ghost" onClick={handleReferralClick} className="flex items-center gap-1.5 px-2 py-1.5 bg-green-500/10 rounded-full border border-green-500/20 shadow-sm hover:bg-green-500/15 transition-all duration-200 active:scale-95">
                <Gift className="w-3 h-3 text-green-600 dark:text-green-400 animate-bounce-subtle" />
                <span className="text-xs font-semibold text-green-600 dark:text-green-400">
                  Refer
                </span>
              </Button>
            </>) : (/* Logged out - Show Sign Up prompt */
        <Button size="sm" onClick={handleSignUpPrompt} className="bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-primary-foreground font-medium px-4 py-2 text-xs rounded-full shadow-lg hover:shadow-xl transition-all duration-300 active:scale-95 animate-glow-pulse">
              <Gift className="w-3 h-3 mr-1.5 animate-bounce-subtle" />
              Get 50FC on Sign Up
            </Button>)}
        </div>
      </div>
