...
const PackageCarousel = () => {
  ...
  return <section className="section-padding-lg bg-muted/30">
      <div className="container mx-auto container-padding py-[25px]">
        {/* Section Header */}
        <div className="text-center mb-4 sm:mb-6 lg:mb-8 animate-fade-in py-0">
        ...
        {activePackages.length === 0 ? <div className="text-center py-8 sm:py-12 lg:py-16">
      ...
      <div className="relative">
          ...
          loop: true
              ...
              <CarouselContent className="-ml-2 md:-ml-4">
                {activePackages.map(pkg => <CarouselItem key={pkg.id} className={`pl-2 md:pl-4 ${isMobile ? 'basis-full' : 'basis-1/3'}`}>
            ...
            <div className="flex justify-center space-x-2 mt-6">
              {activePackages.map((_, index) => <button key={index} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === current ? 'bg-primary w-6' : 'bg-muted-foreground/30'}`} onClick={() => api?.scrollTo(index)} />)}
            </div>
          </div>)}
      </div>
    </section>;
