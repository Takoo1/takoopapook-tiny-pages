...
const OrganizerDashboard = () => {
  ...
  const fetchTickets = async (gameId: string) => {
    try {
      ...
      const stats = ticketsData?.reduce((acc, ticket) => {
        ...
        return acc;
      }, { totalTickets: 0, offlineSold: 0, onlineSold: 0, available: 0, fortuneCounter: 0 }) || 
      { totalTickets: 0, offlineSold: 0, onlineSold: 0, available: 0, fortuneCounter: 0 };

      // Fetch real fortune counter from database
      try {
        const { data: fortuneCount, error: fortuneError } = await supabase.rpc('get_fortune_counter', { game_id: gameId });
        if (!fortuneError) {
          stats.fortuneCounter = fortuneCount || 0;
        }
      } catch (error) {
        console.error('Error fetching fortune counter:', error);
      }
  ...
  const handleTicketClick = async (ticketId: string, currentStatus: string) => {
    ...
    try {
      ...
      // Update fortune counter from database
      try {
        const { data: fortuneCount, error: fortuneError } = await supabase.rpc('get_fortune_counter', { game_id: gameData.id });
        if (!fortuneError) {
          newStats.fortuneCounter = fortuneCount || 0;
        }
      } catch (error) {
        console.error('Error fetching fortune counter:', error);
      }
  ...
  const handleFortuneCounterClick = () => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background/95 to-muted/50 p-4">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          ...
          <Card 
            ...
            onClick={handleFortuneCounterClick}
          >
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-purple-600">{stats.fortuneCounter}</div>
              <div className="text-sm text-muted-foreground">Fortune Counter</div>
      ...
      {gameData && (
        <FortuneCounterModal
          ...
          fortuneCounter={stats.fortuneCounter}
          isAdmin={false}
