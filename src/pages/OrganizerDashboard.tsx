    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background/95 to-muted/50 p-4">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-lg md:text-2xl font-bold text-foreground">{gameData.title}</h1>
            <p className="text-xs md:text-sm text-muted-foreground">{gameData.organising_group_name}</p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm md:text-lg">Game Information</CardTitle>
          </CardHeader>
          <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-xs md:text-sm"><strong>Description:</strong> {gameData.description || 'No description'}</p>
              <p className="text-xs md:text-sm"><strong>Game Date:</strong> {new Date(gameData.game_date).toLocaleDateString()}</p>
              <p className="text-xs md:text-sm"><strong>Ticket Price:</strong> ₹{gameData.ticket_price}</p>
            </div>
            <div>
              <p className="text-xs md:text-sm"><strong>Ticket Range:</strong> {gameData.starting_ticket_number} - {gameData.last_ticket_number}</p>
              <p className="text-xs md:text-sm"><strong>Total Tickets:</strong> {gameData.total_tickets}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-xl md:text-2xl font-bold text-foreground">{stats.totalTickets}</div>
              <div className="text-xs md:text-sm text-muted-foreground">Total Tickets</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-xl md:text-2xl font-bold text-green-600">{stats.onlineSold}</div>
              <div className="text-xs md:text-sm text-muted-foreground">Online Sold</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-xl md:text-2xl font-bold text-gray-600">{stats.available}</div>
        ...
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <CardTitle className="text-sm md:text-lg">Lottery Tickets</CardTitle>
